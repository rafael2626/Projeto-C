...
typedef struct questionario  //ESTRUTURA LIMITE DE 1000 QUESTIONARIOS 200 POR ESCOLA
{
    ...
    char tipo_participante[TOTALQUESTIONARIOS];         //SE � (DOCENTE,ESTUDANTE,FUNCION�RIO OU INVESTIGADOR)
    char contacto_com_pessoa[TOTALQUESTIONARIOS];            //SE A PRESSOA TEVE EM CONTACTO COM ALGUEM COM COVID (SIM/NAO/DESCONHECIDO)
    char sintomas[TOTALQUESTIONARIOS][MAX_TEXTO];       //QUE SINTOMAS A PESSOA A SER QUESTIONADA TEM (DIFICULDADE RESPIRATORIA,,DOR MUSCULAR,FADIGA,FALTA DE PALADAR,N�USEAS e TOSSE)
    char doencas[TOTALQUESTIONARIOS][MAX_TEXTO];  //DOEN�AS DE RISCO (AUTOIMUNE,CANCRO,CARDIOVASCULAR,DIABETES,HIPERTENS�O,OBESIDADE,OBESIDADE,RENAL,RESPIRAT�RIA)
...
char  menu_opcao_contacto_com_pessoa_infetada(void) //AQUI � INT
{
    char opcao;
    do
    {
...
void menu_questionario(void)
{
    ...
    printf("5.Nos �ltimos 15 dias esteve em contacto com pessoa infetada com COVID-19\n");
    printf("6.Que sintomas sente \n");
    printf("7.Que doen�as tem\n");
...
int ler_idade_em_ficheiros(t_questionario idade[])
{
    ...
    {
        printf("Imposs�vel ler ficheiros");
    }
    else
    {
...
{
    //TODO: FAZER FUN��ES PARA ESTAS CENAS
    printf("ID:\n");
    printf("ID ESCOLA:\n");
    printf("DATA:");
    printf("HORA:\n");
